/**
 * Generate simple keyword-based embeddings for policy chunks
...
const KEYWORD_GROUPS = [
  // Device/Phone related (0-19)
  ['phone', 'mobile', 'device', 'smartphone', 'call', 'text', 'confiscate', 'bag', 'pocket'],
  ['photography', 'photo', 'video', 'camera', 'record', 'image', 'picture', 'film'],
  // Safety/Reporting (20-39)
  ['safeguarding', 'dsl', 'designated', 'lead', 'concern', 'report', 'welfare'],
  ['cyberbullying', 'bully', 'harassment', 'online', 'abuse', 'threat'],
  // AI/Academic (40-69)
  ['ai', 'chatgpt', 'artificial', 'intelligence', 'copilot', 'generative'],
  ['homework', 'assignment', 'coursework', 'essay', 'project', 'submission'],
  ['plagiarism', 'copy', 'cheat', 'misconduct', 'academic', 'integrity'],
  ['citation', 'cite', 'reference', 'acknowledge', 'source', 'attribution'],
  // IT/Network (70-99)
  ['password', 'login', 'credential', 'account', 'security', 'authentication'],
  ['website', 'internet', 'browse', 'access', 'blocked', 'filter', 'restriction'],
  ['wifi', 'network', 'bandwidth', 'streaming', 'download'],
  ['software', 'install', 'application', 'app', 'game', 'program'],
  // Data Protection (100-129)
  ['data', 'personal', 'information', 'privacy', 'gdpr'],
  ['access', 'request', 'sar', 'subject', 'rights'],
  ['retention', 'storage', 'keep', 'delete', 'archive'],
  ['breach', 'incident', 'ico', 'notification', 'leak'],
  // Consequences (130-149)
  ['consequence', 'sanction', 'punishment', 'disciplinary', 'action'],
  ['warning', 'detention', 'suspension', 'exclusion', 'expulsion'],
  ['parent', 'guardian', 'contact', 'notify', 'inform'],
  // General education (150-169)
  ['student', 'pupil', 'school', 'class', 'lesson', 'teacher', 'staff'],
  ['rule', 'policy', 'guideline', 'requirement', 'must', 'should', 'permitted']
]
...
function main() {
  console.log('Generating keyword-based embeddings for', POLICY_CONTENT.length, 'chunks...')

  const chunks = POLICY_CONTENT.map((policy, index) => {
    console.log(`Processing: ${policy.source} - ${policy.section}`)

    return {
      id: `chunk-${index}`,
      content: policy.content,
      embedding: generateEmbedding(policy.content),
      metadata: {
